...
var ChatConversationalAgent = class ChatConversationalAgent extends require_agent.Agent {
	...
	*/
	static createPrompt(tools, args) {
		...
		const formatInstructions = (args?.outputParser ?? ChatConversationalAgent.getDefaultOutputParser({ toolNames })).getFormatInstructions({ toolNames });
		const renderedHumanMessage = (0, _langchain_core_prompts.renderTemplate)(humanMessage, "f-string", {
			format_instructions: formatInstructions,
			tools: toolStrings
		});
		const messages = [
			_langchain_core_prompts.SystemMessagePromptTemplate.fromTemplate(systemMessage),
			new _langchain_core_prompts.MessagesPlaceholder("chat_history"),
			_langchain_core_prompts.HumanMessagePromptTemplate.fromTemplate(renderedHumanMessage),
