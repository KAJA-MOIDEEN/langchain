...
function _convertAgentStepToMessages(action, observation) {
	if (isFunctionsAgentAction(action) && action.messageLog !== void 0) return action.messageLog?.concat(new _langchain_core_messages.FunctionMessage(observation, action.tool));
...
var OpenAIAgent = class OpenAIAgent extends require_agent.Agent {
	...
	*/
	static createPrompt(_tools, fields) {
		...
		return _langchain_core_prompts.ChatPromptTemplate.fromMessages([
			_langchain_core_prompts.SystemMessagePromptTemplate.fromTemplate(prefix),
			new _langchain_core_prompts.MessagesPlaceholder("chat_history"),
			_langchain_core_prompts.HumanMessagePromptTemplate.fromTemplate("{input}"),
			new _langchain_core_prompts.MessagesPlaceholder("agent_scratchpad")
		]);
	}
	/**
	...
	*/
	async plan(steps, inputs, callbackManager) {
		...
		const valuesForLLM = { functions: this.tools.map((tool) => (0, _langchain_core_utils_function_calling.convertToOpenAIFunction)(tool)) };
...
async function createOpenAIFunctionsAgent({ llm, tools, prompt, streamRunnable }) {
	...
	const llmWithTools = llm.bindTools ? llm.bindTools(tools) : llm.withConfig({ functions: tools.map((tool) => (0, _langchain_core_utils_function_calling.convertToOpenAIFunction)(tool)) });
	return require_agent.AgentRunnableSequence.fromRunnables([
		_langchain_core_runnables.RunnablePassthrough.assign({ agent_scratchpad: (input) => require_agents_format_scratchpad_openai_functions.formatToOpenAIFunctionMessages(input.steps) }),
		prompt,
		llmWithTools,
