...
* @augments Agent
*/
var StructuredChatAgent = class StructuredChatAgent extends Agent {
	static lc_name() {
	...
	*/
	static getDefaultOutputParser(fields) {
		if (fields?.llm) return StructuredChatOutputParserWithRetries.fromLLM(fields.llm, { toolNames: fields.toolNames });
		return new StructuredChatOutputParserWithRetries({ toolNames: fields?.toolNames });
	}
	/**
...
async function createStructuredChatAgent({ llm, tools, prompt, streamRunnable }) {
	...
	let toolNames = [];
	if (tools.every(isOpenAITool)) toolNames = tools.map((tool) => tool.function.name);
	else if (tools.every(isStructuredTool)) toolNames = tools.map((tool) => tool.name);
	...
	return AgentRunnableSequence.fromRunnables([
		RunnablePassthrough.assign({ agent_scratchpad: (input) => formatLogToString(input.steps) }),
		partialedPrompt,
		llmWithStop,
		StructuredChatOutputParserWithRetries.fromLLM(llm, { toolNames })
	], {
...
//#endregion
export { StructuredChatAgent, createStructuredChatAgent };
