...
var XMLAgent = class XMLAgent extends require_agent.BaseSingleActionAgent {
	...
	static createPrompt() {
		return _langchain_core_prompts.ChatPromptTemplate.fromMessages([_langchain_core_prompts.HumanMessagePromptTemplate.fromTemplate(require_prompt.AGENT_INSTRUCTIONS), _langchain_core_prompts.AIMessagePromptTemplate.fromTemplate("{intermediate_steps}")]);
	}
	/**
...
async function createXmlAgent({ llm, tools, prompt, streamRunnable }) {
	...
	return require_agent.AgentRunnableSequence.fromRunnables([
		_langchain_core_runnables.RunnablePassthrough.assign({ agent_scratchpad: (input) => require_agents_format_scratchpad_xml.formatXml(input.steps) }),
		partialedPrompt,
		llmWithStop,
