...
const require_openapi = require("../../util/openapi.cjs");
...
function convertOpenAPISchemaToJSONSchema(schema, spec) {
	...
	if (schema.type === "array") return {
		type: "array",
		items: convertOpenAPISchemaToJSONSchema(spec.getSchema(schema.items ?? {}), spec),
	...
	for (const path of Object.keys(spec.document.paths)) {
		...
		for (const method of spec.getMethodsForPath(path)) {
			...
			if (requestBody?.content !== void 0) {
				...
				for (const [mediaType, mediaTypeObject] of Object.entries(requestBody.content)) if (mediaTypeObject.schema !== void 0) requestBodySchemas[mediaType] = convertOpenAPISchemaToJSONSchema(spec.getSchema(mediaTypeObject.schema), spec);
			...
			const openAIFunction = {
				name: require_openapi.OpenAPISpec.getCleanedOperationId(operation, path, method),
...
*/
var SimpleRequestChain = class extends require_base.BaseChain {
	static lc_name() {
...
async function createOpenAPIChain(spec, options = {}) {
	...
	if (typeof spec === "string") try {
		convertedSpec = await require_openapi.OpenAPISpec.fromURL(spec);
	} catch {
		try {
			convertedSpec = require_openapi.OpenAPISpec.fromString(spec);
		} catch {
			throw new Error(`Unable to parse spec from source ${spec}.`);
		}
	}
	else convertedSpec = require_openapi.OpenAPISpec.fromObject(spec);
	...
	const { llm = options.llm, prompt = _langchain_core_prompts.ChatPromptTemplate.fromMessages([_langchain_core_prompts.HumanMessagePromptTemplate.fromTemplate("Use the provided API's to respond to this user query:\n\n{query}")]), requestChain = new SimpleRequestChain({ requestMethod: async (name, args) => defaultExecutionMethod(name, args, {
		headers: options.headers,
		params: options.params
	}) }), llmChainInputs = {}, verbose, ...rest } = options;
	const formatChain = new require_llm_chain.LLMChain({
		llm,
		prompt,
		outputParser: new require_openai_functions.JsonOutputFunctionsParser({ argsOnly: false }),
		outputKey: "function",
		llmKwargs: { functions: openAIFunctions },
		...llmChainInputs
	});
	return new require_sequential_chain.SequentialChain({
