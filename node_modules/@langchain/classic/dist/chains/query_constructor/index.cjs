	...
	*/
	async outputProcessor({ query, filter }) {
		...
		if (filter === "NO_FILTER" || filter === void 0) return new _langchain_core_structured_query.StructuredQuery(query);
			...
			return new _langchain_core_structured_query.StructuredQuery(query, parsedFilter);
		}
	}
	/**
...
function _getPrompt(documentContents, attributeInfo, allowedComparators, allowedOperators, examples = defaultExample) {
	...
	const schema = (0, _langchain_core_prompts.interpolateFString)(require_prompt.DEFAULT_SCHEMA, {
		allowed_comparators: myAllowedComparators.join(" | "),
		allowed_operators: myAllowedOperators.join(" | ")
	});
	const prefix = (0, _langchain_core_prompts.interpolateFString)(require_prompt.DEFAULT_PREFIX, { schema });
	const suffix = (0, _langchain_core_prompts.interpolateFString)(require_prompt.DEFAULT_SUFFIX, {
		i: examples.length + 1,
