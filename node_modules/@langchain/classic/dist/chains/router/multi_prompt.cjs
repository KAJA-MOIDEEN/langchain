...
var MultiPromptChain = class MultiPromptChain extends require_multi_route.MultiRouteChain {
	...
	*/
	static fromLLMAndPrompts(llm, { promptNames, promptDescriptions, promptTemplates, defaultChain, llmChainOpts, conversationChainOpts, multiRouteChainOpts }) {
		...
		const routerTemplate = (0, _langchain_core_prompts.interpolateFString)(require_multi_prompt_prompt.STRUCTURED_MULTI_PROMPT_ROUTER_TEMPLATE(outputParser.getFormatInstructions({ interpolationDepth: 4 })), { destinations: destinationsStr });
		const routerPrompt = new _langchain_core_prompts.PromptTemplate({
		...
		const destinationChains = require_utils.zipEntries(promptNames, promptTemplates).reduce((acc, [name, template]) => {
			...
			else if (typeof template === "string") myPrompt = new _langchain_core_prompts.PromptTemplate({
				template,
				inputVariables: ["input"]
			});
