...
var SqlDatabaseChain = class extends require_base.BaseChain {
	...
	async _call(values, runManager) {
		...
		let inputText = `${values[this.inputKey]}\nSQLQuery:`;
	...
	*/
	async verifyNumberOfTokens(inputText, tableinfo) {
		...
		const llm = this.llm;
		const stringWeSend = `${inputText}${this.prompt.template}${tableinfo}`;
		if (await (0, _langchain_core_language_models_base.calculateMaxTokens)({
			prompt: stringWeSend,
			modelName: llm.model
		}) < (llm.maxTokens ?? -1)) throw new Error(`The combination of the database structure and your question is too big for the model ${llm.model} which can compute only a max tokens of ${(0, _langchain_core_language_models_base.getModelContextSize)(llm.model)}.
...
async function createSqlQueryChain({ llm, db, prompt, k = 5, dialect }) {
	...
	if (difference(/* @__PURE__ */ new Set([
		"input",
		"top_k",
		"table_info"
	...
	return _langchain_core_runnables.RunnableSequence.from([
		_langchain_core_runnables.RunnablePassthrough.assign({
			input: (x) => {
