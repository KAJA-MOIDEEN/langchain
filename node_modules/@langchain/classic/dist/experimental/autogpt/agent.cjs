...
var AutoGPT = class AutoGPT {
	...
	constructor({ aiName, memory, chain, outputParser, tools, feedbackTool, maxIterations }) {
		...
		const chunkSize = (0, _langchain_core_language_models_base.getEmbeddingContextSize)("modelName" in memory.vectorStore.embeddings ? memory.vectorStore.embeddings.modelName : void 0);
	...
	*/
	static fromLLMAndTools(llm, tools, { aiName, aiRole, memory, maxIterations = 100, outputParser = new require_output_parser.AutoGPTOutputParser() }) {
		const prompt = new require_prompt.AutoGPTPrompt({
			...
			sendTokenLimit: (0, _langchain_core_language_models_base.getModelContextSize)("modelName" in llm ? llm.modelName : "gpt2")
		});
	...
	*/
	async run(goals) {
		...
		while (loopCount < this.maxIterations) {
			...
			this.fullMessageHistory.push(new _langchain_core_messages.HumanMessage(user_input));
			this.fullMessageHistory.push(new _langchain_core_messages.AIMessage(assistantReply));
			const action = await this.outputParser.parse(assistantReply);
			const tools = this.tools.reduce((acc, tool) => ({
				...acc,
				[tool.name]: tool
			}), {});
			if (action.name === "finish") return action.args.response;
			let result;
			...
			this.fullMessageHistory.push(new _langchain_core_messages.SystemMessage(result));
		}
	}
};
//#endregion
