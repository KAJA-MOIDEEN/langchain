...
var EntityMemory = class extends require_memory_chat_memory.BaseChatMemory {
	...
	*/
	async loadMemoryVariables(inputs) {
		const promptInputKey = this.inputKey ?? (0, _langchain_core_memory.getPromptInputKey)(inputs, this.memoryVariables);
		const messages = await this.chatHistory.getMessages();
		const serializedMessages = (0, _langchain_core_messages.getBufferString)(messages.slice(-this.k * 2), this.humanPrefix, this.aiPrefix);
