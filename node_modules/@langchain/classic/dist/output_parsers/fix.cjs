...
*/
var OutputFixingParser = class OutputFixingParser extends _langchain_core_output_parsers.BaseOutputParser {
	static lc_name() {
		...
		try {
			...
			if (e instanceof _langchain_core_output_parsers.OutputParserException) {
				...
				if (isLLMChain(this.retryChain)) {
					const newCompletion = (await this.retryChain.call(retryInput, callbacks))[this.retryChain.outputKey];
					return this.parser.parse(newCompletion, callbacks);
				} else return await this.retryChain.invoke(retryInput, { callbacks });
			}
			throw e;
		}
	}
