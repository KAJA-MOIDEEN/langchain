...
var WebBrowser = class extends _langchain_core_tools.Tool {
	...
	async _call(inputs, runManager) {
		...
		if (doSummary) context = texts.slice(0, 4).join("\n");
			...
			const results = await (await require_vectorstores_memory.MemoryVectorStore.fromDocuments(docs, this.embeddings)).similaritySearch(task, 4, void 0, runManager?.getChild("vectorstore"));
