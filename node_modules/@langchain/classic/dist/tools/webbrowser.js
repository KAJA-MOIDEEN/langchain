...
var webbrowser_exports = /* @__PURE__ */ __exportAll({
...
var WebBrowser = class extends Tool {
	...
	async _call(inputs, runManager) {
		...
		if (doSummary) context = texts.slice(0, 4).join("\n");
			...
			const results = await (await MemoryVectorStore.fromDocuments(docs, this.embeddings)).similaritySearch(task, 4, void 0, runManager?.getChild("vectorstore"));
		...
		return RunnableSequence.from([this.model, new StringOutputParser()]).invoke(input, runManager?.getChild());
	}
	name = "web-browser";
