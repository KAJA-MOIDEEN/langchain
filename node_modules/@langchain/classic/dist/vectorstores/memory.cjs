...
* <br />
*/
var MemoryVectorStore = class MemoryVectorStore extends _langchain_core_vectorstores.VectorStore {
	memoryVectors = [];
	similarity;
	_vectorstoreType() {
		return "memory";
	...
	*/
	...
	async _queryVectors(query, k, filter) {
		const filterFunction = (memoryVector) => {
			...
			return filter(new _langchain_core_documents.Document({
				metadata: memoryVector.metadata,
				pageContent: memoryVector.content,
				id: memoryVector.id
			}));
		};
		return this.memoryVectors.filter(filterFunction).map((vector, index) => ({
	...
	*/
	async similaritySearchVectorWithScore(query, k, filter) {
		return (await this._queryVectors(query, k, filter)).map((search) => [new _langchain_core_documents.Document({
	...
	async maxMarginalRelevanceSearch(query, options) {
		...
		return (0, _langchain_core_utils_math.maximalMarginalRelevance)(queryEmbedding, embeddingList, options.lambda, options.k).map((idx) => new _langchain_core_documents.Document({
